...
export function EventLogsTab() {
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with Raphael
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('Raphael - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by Raphael', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`raphael-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <span className="i-ph:download text-lg text-gray-500 dark:text-gray-400 group-hover:text-cyan-500 transition-colors" />
          Export
        </button>
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-cyan-500/10 dark:hover:bg-cyan-500/20 cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-cyan-500 transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-cyan-500 transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span className="i-ph:arrows-clockwise text-lg text-gray-500 dark:text-gray-400 group-hover:text-cyan-500 transition-colors" />
            Refresh
          </button>
